...
# THREAD-SAFE HYBRID LSTM + XGBOOST + SENTIMENT (YAHOO FINANCE) — Guarded shapes
import os
import threading
import numpy as np
import pandas as pd
...
def get_stock_news_sentiment(ticker: str) -> pd.DataFrame:
    """
    Returns DataFrame with columns ['date', 'score'] where score ∈ {-1, 0, 1}.
    Uses Yahoo Finance news (free, unlimited).
    """
    try:
        ...
        for item in news_items[:10]:  # Limit to 10 most recent
            ...
            if not title:
                continue
            result = _sentiment_pipeline(title)[0]
...
def get_stock_historical_data(ticker: str, days: int = 90) -> pd.DataFrame:
    ...
    try:
        ...
        if df.empty:
            # Method 2: Use date range
            end = datetime.now()
            start = end - timedelta(days=days)
            df = stock.history(start=start, end=end, interval="1d")
        if df.empty:
            # Method 3: Direct download
            df = yf.download(ticker.upper(), period=f"{days}d", interval="1d", progress=False)
        ...
        if not required_cols.issubset(df.columns):
            print(f"[Yahoo] Missing columns. Found: {list(df.columns)}")
            return pd.DataFrame()

        return df[["Date", "Open", "High", "Low", "Close", "Volume"]].copy()

    except Exception as e:
        print(f"[Yahoo] Error fetching data for {ticker}: {e}")
        return pd.DataFrame()
...
def prepare_data(df: pd.DataFrame, sentiment_df: pd.DataFrame):
    ...
    if df.empty:
        raise ValueError("No usable rows after feature engineering (insufficient data).")
    ...
    y_2d = y.reshape(-1, 1)
    y_scaled_2d = scaler_y.fit_transform(y_2d)
...
def train_lstm(X, y_scaled_2d):
    xgb, tf_dummy, Sequential, LSTM, Dense = _lazy_imports()
...
def train_xgboost(X, y_1d):
    xgb, *_ = _lazy_imports()
    if y_1d.ndim != 1:
        raise ValueError(f"XGBoost expects 1D y, got {y_1d.shape}")
...
def predict_future(X, lstm_model, xgb_model, scaler_y, dates, future_days=10):
    ...
    for _ in range(future_days):
        # LSTM prediction (3D input)
        lstm_pred = safe_predict(lstm_model, last_X.reshape(1, 1, -1))
        try:
            lstm_val = float(np.asarray(lstm_pred).flatten()[0])
        except:
            raise ValueError(f"Invalid LSTM output: {lstm_pred}")

        # Inject into next feature vector
        new_X = np.roll(last_X, -1)
        new_X[-1] = lstm_val  # replace score with LSTM output

        # XGBoost prediction
        xgb_pred = xgb_model.predict(new_X.reshape(1, -1))[0]
        future_scaled.append(xgb_pred)
        last_X = new_X

    # Inverse scale
    future_scaled_arr = np.array(future_scaled).reshape(-1, 1)
    future_prices = scaler_y.inverse_transform(future_scaled_arr).flatten()
...
def generate_stock_prediction(ticker: str, days: int = 10):
    """
    Main function: returns prediction dict.
    """
    ...
    return {
        ...
        "dates": [d.isoformat() for d in predictions_df["Date"]],
        "model": "Hybrid LSTM + XGBoost + Sentiment (Yahoo Finance)"
    }